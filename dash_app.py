...
from code_prompt_generator import CodeGenerationPromptGenerator
    ...
    if "generate-method" in trigger_id:
        ...
        try:
            ...
            output = html.Div([
                ...
                dbc.Button(
                    "🐍 Generate Python",
                    id={'type': 'generate-python-method', 'class': class_name, 'method': method_name},
                    color="success",
                    size="sm",
                    className="mt-3",
                    style={"marginTop": "12px"}
                ),
                # NEW: Spinner for method python generation
                dbc.Spinner(
                    html.Div(
                        id={'type': 'method-python-spinner', 'class': class_name, 'method': method_name},
                        style={"display": "none"}
                    ),
                    color="success",
                    size="sm",
                ...
                # Python code output container
                html.Div(
                    id={'type': 'method-python-output', 'class': class_name, 'method': method_name},
                    style={"marginTop": "12px"}
                )
            ])
    ...
    if "generate-field" in trigger_id:
        ...
        try:
            ...
            output = html.Div([
                ...
                dbc.Button(
                    "🐍 Generate Python",
                    id={'type': 'generate-python-field', 'class': class_name, 'field': field_name},
                    color="success",
                    size="sm",
                    className="mt-3",
                    style={"marginTop": "12px"}
                ),
                # NEW: Spinner for field python generation
                dbc.Spinner(
                    html.Div(
                        id={'type': 'field-python-spinner', 'class': class_name, 'field': field_name},
                        style={"display": "none"}
                    ),
                    color="success",
                    size="sm",
                ...
                # Python code output container
                html.Div(
                    id={'type': 'field-python-output', 'class': class_name, 'field': field_name},
                    style={"marginTop": "12px"}
                )
            ])
...
# Combined callback for method python generation with spinner management
@app.callback(
    [Output({'type': 'method-python-output', 'class': MATCH, 'method': MATCH}, 'children'),
     Output({'type': 'method-python-spinner', 'class': MATCH, 'method': MATCH}, 'style')],
    Input({'type': 'generate-python-method', 'class': MATCH, 'method': MATCH}, 'n_clicks'),
    State('completed-docs-store', 'data'),
    State('parsed-store', 'data'),
    State('source-lines-store', 'data'),
    State({'type': 'method-python-output', 'class': MATCH, 'method': MATCH}, 'id'),
    prevent_initial_call=True
)
def generate_python_method(n_clicks, completed_docs, parsed_data, source_lines, output_id):
    """Generate Python code from Java method documentation with spinner management."""
    if not n_clicks or not completed_docs or not parsed_data:
        raise dash.exceptions.PreventUpdate
    ...
    try:
        ...
        if not docs:
            return html.Pre("No documentation available for code generation.", style={"color": "orange"}), {"display": "none"}
        
        # Use the first doc for code generation
        method_doc = docs[0]
        
        # Convert documentation to JSON string
        documentation_json_string = json.dumps(method_doc, indent=2)
        
        # Generate Python code using ONLY the documentation JSON string
        code_generator = CodeGenerationPromptGenerator()
        prompt = code_generator.build_prompt(documentation_json_string)
        python_codes = code_generator.run_all_prompts([prompt])
        
        if not python_codes:
            return html.Pre("Failed to generate Python code.", style={"color": "red"}), {"display": "none"}
        
        python_code_obj = python_codes[0]
        python_code_dict = python_code_obj.to_dict()
        
        # Display generated Python code
        output = html.Div([
            html.H6("🐍 Generated Python Code:", style={"marginTop": "12px", "color": "#fd7e14", "fontWeight": "bold"}),
            html.Pre(
                python_code_dict.get("python_code", ""),
                style={
                    "whiteSpace": "pre-wrap",
            ...
            html.H6("📝 Explanation:", style={"marginTop": "12px", "color": "#6c757d", "fontWeight": "bold"}),
            html.Div(
                python_code_dict.get("explanation", ""),
                style={
                    "padding": "12px",
                    "backgroundColor": "#f8f9fa",
                    "borderRadius": "4px",
        ...
        # Hide spinner after successful generation
        return output, {"display": "none"}
        
    except Exception as e:
        # Hide spinner on error
        return html.Pre(f"❌ Error generating Python code: {str(e)}", style={"color": "red"}), {"display": "none"}


# Combined callback for field python generation with spinner management
@app.callback(
    [Output({'type': 'field-python-output', 'class': MATCH, 'field': MATCH}, 'children'),
     Output({'type': 'field-python-spinner', 'class': MATCH, 'field': MATCH}, 'style')],
    Input({'type': 'generate-python-field', 'class': MATCH, 'field': MATCH}, 'n_clicks'),
    State('completed-docs-store', 'data'),
    State('parsed-store', 'data'),
    State('source-lines-store', 'data'),
    State({'type': 'field-python-output', 'class': MATCH, 'field': MATCH}, 'id'),
    prevent_initial_call=True
)
def generate_python_field(n_clicks, completed_docs, parsed_data, source_lines, output_id):
    """Generate Python code from Java field documentation with spinner management."""
    if not n_clicks or not completed_docs or not parsed_data:
        raise dash.exceptions.PreventUpdate
    ...
    try:
        ...
        # Convert documentation to JSON string
        documentation_json_string = json.dumps(field_doc, indent=2)
